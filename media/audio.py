...
#        if not self.ffmpeg.loaded: raise WrongOrderError("Stream is not loaded.")
        ...
        self.sdStream=Sounddevice(mode=self.mode, dataQueue=self.ffmpeg._audioQ, streamOptions={"samplerate":info["sample_rate"], "blocksize":info["frame_size"], "channels":self.channels, "device":self.device})
